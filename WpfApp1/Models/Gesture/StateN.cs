...
public class StateN : State
{
    public readonly GestureMachineAbs Machine;
    ...
    public override Result Input(IPhysicalEvent evnt)
    {
        if (evnt is PhysicalFireEvent fireEvent && IsSingleThrowTrigger(fireEvent))
        {
            ...
            return StateResult.Create(true, ToNonCancellableClone());
        }
        else if (evnt is PhysicalPressEvent pressEvent)
        {
            if (IsRepeatedStartTrigger(pressEvent))
            {
                return StateResult.Create(true, this);
            }
            else if (IsDoubleThrowTrigger(pressEvent))
            {
                    ...
                    HasReleaseExecutors(nextDoubleThrowElements))
                {
                    ...
                    return StateResult.Create(true,
        ...
        else if (evnt is PhysicalReleaseEvent releaseEvent)
        {
            ...
            if (IsNormalEndTrigger(releaseEvent))
            {
                ...
                else if (CanCancel)
                {
                    ...
                    return StateResult.Create(true, LastState);
                }

                if (LastState is StateN stateN)
                {
                    return StateResult.Create(true, stateN.ToNonCancellableClone());
                }
            ...
            else if (IsAbnormalEndTrigger(releaseEvent))
            {
                ...
                if (!CanCancel &&
                    queryResult.FoundState is StateN stateN)
                {
                    return StateResult.Create(true, stateN.ToNonCancellableClone());
                }

                return StateResult.Create(true, queryResult.FoundState);
            }
    ...
    private StateN ToNonCancellableClone()
        => new(
            Machine, Ctx, History, DoubleThrowElements, Depth, false);

    private StateN CreateCancellableNextState(PhysicalPressEvent pressEvent,
        IReadOnlyList<IReadOnlyDoubleThrowElement> doubleThrowElements)
        => new(Machine, Ctx, History.CreateNext(pressEvent.Opposition, this), doubleThrowElements, Depth + 1, true);

    private StateN CreateNonCancellableNextState(PhysicalPressEvent pressEvent,
        IReadOnlyList<IReadOnlyDoubleThrowElement> doubleThrowElements)
        => new(Machine, Ctx, History.CreateNext(pressEvent.Opposition, ToNonCancellableClone()), doubleThrowElements,
            Depth + 1, false);
    ...
    public IReadOnlyList<HistoryRecord>
        CreateHistory(
    ...
    private static IReadOnlyCollection<PhysicalReleaseEvent>
        GetEndTriggers(IReadOnlyList<HistoryRecord> history)

...
public class MainFormBase : MouseGestureForm
{
    ...
    public void UpdateTasktrayMessage(IReadOnlyList<GestureMachineProfile> profiles)
    {
        ...
        if (profiles.Count > 1)
        {
            var perProfileMessages = Enumerable.Range(0, profiles.Count)
                .Select(n =>
                    profiles.Take(n + 1).Select(p => $"({p.ProfileName}): {p.RootElement.GestureCount}")
                        .Aggregate("", (a, b) => a + "\r\n" + b)).Select(s => s.Trim()).Reverse().ToList();

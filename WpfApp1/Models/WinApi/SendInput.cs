...
public class InputSender
{
    ...
    {
        [FieldOffset(0)] public MOUSEINPUT asMouseInput;
        [FieldOffset(0)] public KEYBDINPUT asKeyboardInput;
        [FieldOffset(0)] public HARDWAREINPUT asHardwareInput;
    }
    ...
    private readonly UIntPtr MOUSEEVENTF_CREVICE_APP = new(0xFFFFFF00);
    private readonly UIntPtr KEYBOARDEVENTF_CREVICE_APP = new(0xFFFFFF00);
    ...
    protected void Send(INPUT[] input)
    {
        ...
        foreach (var item in input.Select((v, i) => new { v, i, }))
        {
            ...
            if (inputType.Equals(InputType.INPUT_MOUSE))
            {
                ...
                {
                    log.Add(
                        $"mouseData: {(XButtonType)data.mouseData.asXButton.type} | {ToHexString((uint)data.mouseData.asXButton.type)}");
                }
                else if (eventType.HasFlag(MouseEventType.MOUSEEVENTF_WHEEL | MouseEventType.MOUSEEVENTF_HWHEEL))
                {
                    log.Add(
                        $"mouseData: {data.mouseData.asWheelDelta.delta} | {ToHexString((uint)data.mouseData.asWheelDelta.delta)}");
                }
    ...
    {
        var point = Window.GetPhysicalCursorPos();
        var x = point.X + dx;
        var y = point.Y + dy;
...
public class SingleInputSender : InputSender
{
    ...
    public InputSequenceBuilder Multiple() => new();
}
